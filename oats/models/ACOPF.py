...
model.epsQ = Var(model.B,within=NonNegativeReals)
# --- cost function ---
def objective(model):
    ...
    sum(model.VOLL[d]*(1-model.alpha[d])*model.baseMVA*model.PD[d] for d in model.D)+ \
          5000*sum(model.epsQ[b] for b in model.B)
    return obj
...
def KCL_reactive_def(model, b):
    ...
    sum(model.BB[s]*model.v[b]**2 for s in model.SHUNT if (b,s) in model.SHUNTbs)+model.epsQ[b]
